<template lang="">
    <main class="flex justify-start items-center px-10 max-sm:mt-20">
        <div class="flex flex-col gap-10 px-20 w-full max-sm:px-0">
            ...
            <div class="flex justify-between items-center">
                <h1
                    class="text-3xl max-sm:text-2xl text-black font-semibold underline underline-offset-2 decoration-8 decoration-maingreen">
                    Editais</h1>

                <v-dialog max-width="500">
                    <template v-slot:activator="{ props: activatorProps }">
                        <v-btn v-bind="activatorProps" color="#2F9E40" text="Inserir novo edital"
                            variant="flat"></v-btn>
                    </template>

                    <template v-slot:default="{ isActive }">
                        <v-card title="Inserir Documento">
                            <v-form class="p-4 md:p-5 space-y-4">
                                <v-text-field v-model="docTitle" :counter="10" :rules="[rules.text]"
                                    class="text-maingreen" color="#2F9E40" label="Título do Documento"></v-text-field>

                                <v-file-input accept="image/*" color="#2F9E40" label="Arquivo PDF"
                            ...
                            <v-card-actions>
                                ...
                                <v-btn class="me-4" color="#2F9E40" type="submit" dark @click.prevent="updatePdf()">
                                    Salvar
                                </v-btn>
            
                                <v-btn text="Fechar" color="error" @click="isActive.value = false"></v-btn>
                            </v-card-actions>
                        </v-card>
                    </template>
                </v-dialog>
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div
                                        ...
                                        <button
                                            ...
                                            data-modal-hide="edit-modal" type="button"
                                            @click="updatePdf()">Salvar</button>
                                        <button
                            ...
                            <div class="relative p-4 w-full max-w-2xl max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div
                                        ...
                                        <button
                                            class="text-white bg-maingreen hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
                                            data-modal-hide="delete-modal" type="button"
                                            @click="deletePdf()">Sim</button>
                                        <button
                                            class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-maingreen focus:z-10 focus:ring-4 focus:ring-gray-100"
                                            data-modal-hide="delete-modal" type="button"
                                            @click="setDelete(null, 'clear')">Não</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    import { createEdict, listEdicts, updateEdict, deleteEdict, downloadEdict } from '@/services/EdictService.js';
    ...
    export default {
        ...
        methods: {
            ...
            downloadFile(filename) {
                downloadEdict(filename).then((response) => {
            ...
            deletePdf() {
                deleteEdict(this.inDelete).then((response) => {
            ...
            createPdf() {
                ...
                try {
                    createEdict(formData).then((response) => {
                        console.log(response.data)
